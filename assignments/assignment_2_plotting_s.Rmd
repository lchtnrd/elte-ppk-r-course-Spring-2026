...
ggplot(
  ...
  aes(
    x = expedition_count,
    y = fct_reorder(peak_lumped, expedition_count, .fun = sum),
    fill = season
  )
) +
  geom_col() +
...
walking_data <- commute %>%
  ...
  summarise(total_walking = sum(n, na.rm = TRUE), .groups = "drop")

biking_data <- commute %>%
  filter(mode == "Bike") %>%
  group_by(state_abb, state_region) %>%
  summarise(total_biking = sum(n, na.rm = TRUE), .groups = "drop")

commute_aggregated <- left_join(
  walking_data,
  biking_data,
  by = c("state_abb", "state_region")
)

ggplot(commute_aggregated, aes(x = total_walking, y = total_biking)) +
  geom_point(aes(color = state_region), size = 2) +
  geom_text(aes(label = state_abb), vjust = -0.5) +
